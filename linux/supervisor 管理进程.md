# supervisor 管理进程-linux

Supervisor ([http://supervisord.org](http://supervisord.org/)) 是一个用 [Python](http://www.ttlsa.com/python/) 写的进程管理工具，可以很方便的用来启动、重启、关闭进程（不仅仅是 Python 进程）。除了对单个进程的控制，还可以同时启动、关闭多个进程，比如很不幸的服务器出问题导致所有应用程序都被杀死，此时可以用 supervisor 同时启动所有应用程序而不是一个一个地敲命令启动。

## 安装

Supervisor 可以运行在 [Linux](http://www.ttlsa.com/linux/)、Mac OS X 上。如前所述，supervisor 是 Python 编写的，所以安装起来也很方便，可以直接用 pip :
...
```
[program:usercenter]  ;usercenter 为你的配置名，不要重复
...
上面这个命令会进入 supervisorctl 的 [shell](http://www.ttlsa.com/shell/) 界面，然后可以执行不同的命令了：
